...
WORKDIR /root/
ADD . /root
RUN cd /root/src && go build -o blockchain-token
...
COPY --from=builder --chown=1000:0 /root/src/blockchain-token /home/user
...
ENV BLOCKCHAIN_CONFIG=/config/blockchain.yml
ENTRYPOINT ./blockchain-token
